...
export default function Header({ 
  ...
  const personas = [
    ...
    { id: 'architecture', label: 'Architecture', icon: Layers, isAction: 'architecture' },
    { id: 'figma', label: 'Figma Prototype', icon: ExternalLink, isAction: 'figma' }
  ];

  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm transition-all">
      {/* Top Deep Green Navbar matching Figma */}
      <div className="bg-[#064e3b] text-white px-4 sm:px-8 py-2.5 flex items-center justify-between">
        ...
        {/* Right: Language Pill, Figma Quick Button & Emergency Call Button */}
        <div className="flex items-center space-x-2.5 sm:space-x-3">
          {/* Quick Figma Button */}
          <button
            onClick={() => setOpenFigmaModal(true)}
            className="hidden sm:inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-white/10 hover:bg-white/20 text-white border border-white/20 transition-all"
            title="Inspect Figma Healthcare Management System"
          >
      ...
      {/* Secondary Persona & Channel Bar matching Figma tabs */}
      <div className="bg-white border-b border-slate-200 px-4 sm:px-8 py-2 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <div className="flex items-center space-x-2 text-xs">
          <span className="text-[11px] text-slate-500 font-medium hidden md:inline">Channels:</span>
          <button
            onClick={() => {
              setSimulatorTab('ivr');
